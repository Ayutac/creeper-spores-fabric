...
public class CreeperlingEntity extends PathAwareEntity implements SkinOverlayOwner {
    ...
    @Override
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, EntityData data, NbtCompound tag) {
    ...
    private static void pushOutOfBlocks(Entity self) {
        ...
        self.pushOutOfBlocks(self.getX() - (double)self.getWidth() * 0.35D, bb.minY + 0.5D, self.getZ() + (double)self.getWidth() * 0.35D);
        self.pushOutOfBlocks(self.getX() - (double)self.getWidth() * 0.35D, bb.minY + 0.5D, self.getZ() - (double)self.getWidth() * 0.35D);
        self.pushOutOfBlocks(self.getX() + (double)self.getWidth() * 0.35D, bb.minY + 0.5D, self.getZ() - (double)self.getWidth() * 0.35D);
        self.pushOutOfBlocks(self.getX() + (double)self.getWidth() * 0.35D, bb.minY + 0.5D, self.getZ() + (double)self.getWidth() * 0.35D);
    }
